...
class GitPack(object):
    ...
    def extract_pack(self):
        sort_objects = sorted(list(self.objects.items()), key=lambda x: x[1]['offset'])
    ...
    def parse_pack(self):
        for _hash, _info in sorted(list(self.objects.items()), key=lambda x: x[1]['offset']):
            _size = []
            try:
    ...
    def pack_to_object_file(self):
        for _object in list(self.objects.values()):
            try:
